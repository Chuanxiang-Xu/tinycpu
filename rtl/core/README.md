...
v0.5-rv32im-m-extension implements fuller RV32I for simple freestanding C plus
...
load/store access are serialized around bus transactions. The M-extension unit
uses a `start`/`busy`/`done` handshake and stalls the current instruction until
the multiply/divide result is ready for writeback.
